...
class Inwave_Shortcode_Class {
    ...
    function enqueue_scripts() {
        ...
        wp_enqueue_style('iw-common', plugins_url() . '/inwave-common/assets/css/iw-shortcodes.css', array(), $theme_info->get('Version'));
        if(is_rtl()){
            wp_enqueue_style('iw-common-rtl', plugins_url() . '/inwave-common/assets/css/iw-shortcodes-rtl.css', array(), $theme_info->get('Version'));
        }
        if(get_option( 'woocommerce_checkout_page_id' ) && get_the_ID() == get_option( 'woocommerce_checkout_page_id' )){
            wp_register_script( 'select2', plugins_url() . '/woocommerce/assets/js/select2/select2.min.js', array( 'jquery' ), '3.5.4' );
        }
        elseif(file_exists(get_template_directory() . '/assets/js/select2.min.js')){
            wp_register_script('select2', get_template_directory_uri() . '/assets/js/select2.min.js', array('jquery'), '1.0.0', true);
        }
        wp_enqueue_script('iw-common', plugins_url() . '/inwave-common/assets/js/iw-shortcodes.min.js', array(), $theme_info->get('Version'), true);

    }

    function enqueue_scripts_admin() {
        $theme_info = wp_get_theme();
        wp_enqueue_style('font-awesome', get_template_directory_uri() . '/assets/fonts/font-awesome/font-awesome.css', array(), $theme_info->get('Version'));
        ...
        wp_enqueue_script('iw-common', plugins_url() . '/inwave-common/assets/js/iw-admin.min.js', array('jquery'), $theme_info->get('Version'), true);
    ...
    function shortcode_add_button() {
        ...
        <div id="iw-shortcode" class="iw-hidden iw-shortcode shortcode-item-view">
            <div class="shortcode-contain">
                <div class="shortcode-control">
                    <div class="title"><?php esc_html_e('Shortcode settings', 'inwavethemes'); ?></div>
                    <div class="close-btn"><i class="fa fa-times"></i></div>
                    <div style="clear: both;"></div>
                </div>
                ...
                <div class="shortcode-save-setting">
                    <div class="save-settings"><?php esc_html_e('Insert Shortcode'); ?></div>
                    <div class="cancel-settings"><?php esc_html_e('Cancel'); ?></div>
                    <div class="preview-settings"><?php esc_html_e('Preview'); ?></div>
    ...
    function get_search_icons_html($type = 'fa', $key = '', $fvalue = '', $limit = 400) {
    ...
    function get_preview_image_settings_field($fname, $fvalue, $class) {
        ob_start();
        ?>
            <a name="<?php echo $fname; ?>" class="preview-image <?php echo $class;?>" href="<?php echo $fvalue; ?>" style="display: inline-block"><img style="max-width: 80px; max-height: 50px" src="<?php echo $fvalue; ?>"></a>
        <?php
        $html = ob_get_contents();
        ob_end_clean();
        return $html;
