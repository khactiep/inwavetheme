    ...
    function inwave_comment($comment, $args, $depth)
    {
        ...
        switch ($comment->comment_type) :
            ...
            <li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
                <div id="comment-<?php comment_ID(); ?>" class="row comment answer">
                    ...
                    <div class="commentRight">
                        <div class="content-cmt">
                            <div class="comment-head-info">
                                ...
                                <div class="comment_reply"><?php comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply ', 'injob'), 'after' => '', 'depth' => $depth, 'max_depth' => $args['max_depth']))); edit_comment_link(esc_html__('| Edit', 'injob'), '<span class="edit-link">', '</span>'); ?></div><br>
								<span class="date-cmt"> <?php echo get_comment_date('d.m.Y').esc_html__(' at ').get_comment_time('h:i a'); ?> </span><br>
                        ...
                        <?php //edit_comment_link(esc_html__('Edit', 'injob'), '<div class="edit-link">', '</div>'); ?>
                    </div>
...
if (!function_exists('inwave_social_sharing')) {
    ...
    function inwave_social_sharing($link, $text, $title, $tag = '')
    {
        ...
        if ($inwave_theme_option['sharing_email']) {
            ...
            $html .= ($tag ? '<' . $tag . '>' : '') . '<a class="social-share-item share-buttons-email" href="' . urlencode(esc_attr($shareLink)) . '" title="' . esc_attr_x('Email','title', 'injob') . '"><i class="fa fa-envelope"></i></a>' . ($tag ? '</' . $tag . '>' : '');
        }
...
if (!function_exists('inwave_social_sharing_category_listing')) {
    ...
    function inwave_social_sharing_category_listing($link, $text, $title, $tag = '')
    {
        ...
        if ($inwave_theme_option['sharing_facebook']) {
            ...
            echo ($tag ? '<' . $tag . '>' : '') . '<a class="share-buttons-fb" target="_blank" href="#" title="' . esc_attr_x('Share on Facebook','title','injob') . '" onclick="return iwOpenWindow(\'' . esc_js($shareLink) . '\')"><i class="fa fa-facebook"></i></a>' . ($tag ? '</' . $tag . '>' : '');
        }
        if ($inwave_theme_option['sharing_google']) {
            $shareLink = 'https://plus.google.com/share?url=' . $link . '&amp;title=' . $title;
    ...
    function inwave_get_social_link()
    {
        ...
        if(isset($inwave_theme_option['social_links']) && count($inwave_theme_option['social_links']) > 1){
            ...
            foreach($social_links as $social_link){
                $html .= '<li><a href="'.esc_url($social_link['link']).'" title="'.esc_attr($social_link['title']).'"><i class="'.esc_attr($social_link['icon']).'"></i></a></li>';
            }
            $html .= '</ul>';
        }
...
if (!function_exists('inwave_social_sharing_fb')) {
    ...
    function inwave_social_sharing_fb($link, $text, $title)
    {
        ...
        $link = urlencode($link);
        $shareLink = 'https://www.facebook.com/sharer.php?s=100&amp;p[title]=' . $title . '&amp;p[url]=' . $link . '&amp;p[summary]=' . $text;
        echo '<a class="share-buttons-fb" target="_blank" href="#" title="' . esc_attr_x('Share on Facebook','title', 'injob') . '" onclick="return iwOpenWindow(\'' . esc_js($shareLink) . '\')"><i class="fa fa-share"></i><span>share</span></a>';
    }
}
...
if(!function_exists('inwave_count_widgets')){
    ...
    function inwave_breadcrumbs(){
        ...
        $text['category'] = esc_html__('Archive by Category "%s"', 'injob'); // text for a category page
        ...
        if ( is_front_page()) {
            ...
            if ( get_query_var('paged') ) {
                ...
                echo esc_html(__('Page', 'injob')) . ' ' . get_query_var('paged');
                if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
            }
